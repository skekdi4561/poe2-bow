...
PROBE_LEVELS = 6          # 무기당 문턱 수. 최소 간격에 걸려 보통 5개가 실제로 돈다.
                          # fetch 는 throttle 이 3초 간격으로 펴므로(한도 100/300초) 문턱을
                          # 늘려도 절벽이 아니라 사이클이 그만큼 길어질 뿐이다(문턱 1개 = 약 3초).
...
def dump_props(item, tag):
    ...
    이걸로 방패 막기가 실려 오는지 확인했다(2026-09-06: 141/141 수집, 막기 옵션이 붙은 행만
    평균 31.9 vs 26.0 으로 높아 값이 진짜인 것까지 확인). 남겨두는 건 GGG 가 속성을 바꿨을 때의 그물이다.
    """
...
def normalize(res, metric="dps"):
    ...
    # 치확 0 은 결함이 아니다. 몽환의 육척봉(Dreaming Quarterstaff)·매끄러운 육척봉
    # (Smooth Quarterstaff)은 **베이스 치확이 0% 인 실제 베이스**다(외부 DB 2곳 확인, 공속 1.50 도 일치).
    # 한때 여기에 덤프를 걸었다가 뺐다 — 정상 상태에 경고를 걸면 재시작마다 떠서 덤프를 무시하게 된다.
    # 근거로 남길 실측(2026-09-06): 수집 86종 2,227개에서 (치확 − 치명타 옵션 합)이 베이스마다
    # 상수였고 흔들리는 베이스가 0종이다. 즉 거래소가 주는 치확은 언제나 베이스+옵션의 최종값이다.
    if metric != "dps":
...
def load_top_dps(page_url, category=None):
    ...
    낮은 DPS 구간은 통째로 뜨지 않고 문턱별 최저가 몇 개(탐침)만 더한다 — probe_levels 참고 — 문턱은 상위 100개 하한에서 유도한다.
            ...
            for tag, nth, lname in LEAGUE_PLAN:
                ...
                for cat_id, suffix, name in ATTACK_WEAPONS:
                    ...
                    try:
                        ...
                        # 반환값의 뜻: collect_weapon 은 0 <=> 파일 미갱신. collect(활)은
                        # 이번 사이클 **신규분**이라, 신규 0 인데 24h 합집합으로 파일이 갱신된
                        # 경우도 0 이다 — 그때 분모가 한 칸 줄지만 '신규 0개'는 어차피 알려야
                        # 할 일이라 그대로 둔다. 반대 방향(파일을 썼는데 0)은 없다: 두 함수 다
                        # 쓰기 전에 빈 결과를 걸러 0 을 조기 반환하므로 거짓 안심은 안 생긴다.
                        if suffix:
